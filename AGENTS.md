...
コミットメッセージは `feat:`, `fix:`, `chore:` などの Conventional Commits を日本語本文で統一し、**1行で簡潔に**記述します。詳細な変更内容はコミット本文ではなくコード差分やプルリクエストで説明してください。必要に応じてチケット番号を後段に付記します。プルリクエストでは概要、テスト結果（実行コマンドやスクリーンショット）、関連 Issue のリンクを記載し、UI 変更時は主要画面のキャプチャを添付してください。レビュワーが確認しやすいよう、差分は機能単位で粒度を揃え、ドラフト状態で早期共有する運用を推奨します。
