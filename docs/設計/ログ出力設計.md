...
- `timestamp`: ISO8601（**JST**, `+09:00` 付き）
...
- `NUXT_LOG_LEVEL`: `info`（既定） / `debug` / `warn` / `error`
- `NUXT_LOG_ENABLE_ACCESS`: `true|false`（既定 `true`。ACCESSログを抑止したい時に使用）

  > **Note**: これらの環境変数は`runtimeConfig`経由で読み込まれます。
...
**Nitroプラグイン**（`server/plugins/logger.ts`）を基点としたロガー実装。

### アーキテクチャ方針

1. **ロガーの初期化と共有**
   - Nitroプラグインでロガーインスタンスを初期化
   - `event.context.logger`を通じて全リクエストで共有
   - `runtimeConfig`から環境変数を取得（`NUXT_LOG_LEVEL`, `NUXT_LOG_ENABLE_ACCESS`）

2. **ログ集約**
   - Nitroフック（`request`, `beforeResponse`, `error`）でアクセスログとエラーログを集約
   - プラグイン内で一元管理し、個別ミドルウェアは不要

3. **カテゴリ別使用**
   - アプリケーションコードでは`event.context.logger.withCategory('API')`等で使い分け
   - カテゴリ別に追加情報（メタデータ）を付与可能

4. **タイムスタンプ**
   - **JST（ISO8601形式）**で出力（例: `2025-11-05T19:30:45+09:00`）
   - `Intl.DateTimeFormat`を使用してタイムゾーン対応
