...
- [ ] HotPepper Gourmet API キー取得
  - [ ] [リクルートWebサービス](https://webservice.recruit.co.jp/)にアクセス
  - [ ] 新規会員登録（未登録の場合）
  - [ ] ログイン後、「新しいアプリケーションを追加」
  - [ ] APIキーを発行（HotPepper Gourmet API v1を選択）
- [ ] Google Maps JavaScript API キー取得
  - [ ] [Google Cloud Console](https://console.cloud.google.com/)にアクセス
  - [ ] 新規プロジェクト作成（未作成の場合）
  - [ ] 「APIとサービス > ライブラリ」から「Maps JavaScript API」を有効化
...
- [ ] `.env`ファイル作成・更新
  - [ ] `.env.example`を`.env`にコピー（未作成の場合）
  - [ ] `NUXT_HOTPEPPER_API_KEY`に取得したHotPepper APIキーを設定
  - [ ] `NUXT_PUBLIC_GOOGLE_MAPS_API_KEY`に取得したGoogle Maps APIキーを設定
  - [ ] その他の環境変数を必要に応じて設定（`NUXT_LOG_LEVEL`等）
- [ ] 開発サーバー再起動
  - [ ] `yarn dev`を再起動して環境変数を読み込む
  - [ ] コンソールでエラーが出ていないことを確認
...
- [ ] `app/pages/index.vue` 更新
  - [ ] モックデータ（`mockGenres`, `mockLargeAreas`, `mockMiddleAreas`, `mockSmallAreas`）を削除
...
- [ ] `app/pages/index.vue` 更新
  - [ ] `handleLocationObtained` 関数を更新
    - [ ] モックデータ生成処理を削除
    - [ ] `useShopSearch()` composableで位置情報検索APIを呼び出し
    - [ ] 位置情報（lat, lng）をパラメータとして渡す
    - [ ] 検索結果を`nearbyShops`に設定
  - [ ] エラーハンドリング（API失敗時の表示）

### 1.3 結合テスト

- [ ] `tests/pages/index.test.ts` 作成（新規ファイル）
  - [ ] マスタデータ取得の確認（MSWでモック）
  - [ ] 位置情報取得〜店舗検索の一連の流れ
  - [ ] ローディング状態の表示確認
  - [ ] エラー時の表示確認
