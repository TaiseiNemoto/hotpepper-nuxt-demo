...
- [x] ~~`types/api.ts`: クライアント用API型~~
  - **不要**: Nuxt 4では`server/types/`の型定義が自動的にクライアント側でも利用可能
  - `server/types/api.ts`, `server/types/hp-internal.ts` を直接利用

### 4.2 Composables実装

- [x] `app/composables/useApi.ts`: 内部API呼び出し共通ロジック
  - `useFetch`のラッパー
  - エラーハンドリング（`ApiResult<T>` 型チェック）
  - ローディング状態管理
- [x] `app/composables/useShopSearch.ts`: HP-01呼び出し用
- [x] `app/composables/useShopDetail.ts`: HP-02呼び出し用
- [x] `app/composables/useMasters.ts`: HP-03〜HP-06呼び出し用

### 4.3 Composables結合テスト

- [ ] **UI実装時に実施**（テスト戦略設計に基づく）
  - Composables単体テストは不要（結合テストで検証）
  - `mountSuspended`を使ったコンポーネント結合テストで以下を検証：
    - 読み込み中 / 0件 / エラー / 正常の状態切替
    - API応答に応じた画面表示
  - 実施タイミング: **「3.2 画面実装」フェーズ**
...
- [x] すべてのチェック項目が完了
- [x] サーバーサイドテストがパス（`yarn test tests/server`）
- [x] 型チェックがパス（`yarn type-check`）
- [x] Lintエラーなし（`yarn lint`）
- [ ] ログ出力が設計仕様どおりに動作することを確認（手動確認）

**注**: Composables結合テストは「3.2 画面実装」フェーズで実施
