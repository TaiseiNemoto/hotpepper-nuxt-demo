...
- [ ] `@nuxtjs/tailwindcss` インストール
  - [ ] `yarn add -D @nuxtjs/tailwindcss`
  - [ ] `nuxt.config.ts` の `modules` に `@nuxtjs/tailwindcss` 追加
- [ ] `tailwind.config.js` 作成
  - [ ] カラーパレット定義（プライマリ/セカンダリ/エラー/成功/警告）
  - [ ] カスタムブレークポイント設定（デスクトップ優先）
  - [ ] フォント設定
- [ ] `app/assets/css/main.scss` 作成（グローバルスタイル）
  - [ ] Tailwind ディレクティブ（`@tailwind base/components/utilities`）
  - [ ] カスタムCSS変数定義（必要に応じて）
  - [ ] リセットCSS・ベーススタイル
- [ ] `nuxt.config.ts` に `css` 配列追加（`~/assets/css/main.scss` を指定）

### 1.2 グローバルスタイル設定

- [ ] フォント読み込み設定（Google Fonts等、必要に応じて）
- [ ] `app.vue` でグローバルスタイルの適用確認
